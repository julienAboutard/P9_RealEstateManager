package com.example.realestatemanager.ui.estate.add.amenity
...
import com.example.realestatemanager.databinding.AmenityCheckboxItemBinding
import com.example.realestatemanager.databinding.AmenityItemBinding
    ...
    sealed class AmenityViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        class AmenityCheckbox(private val binding: AmenityCheckboxItemBinding) :
            ...
            fun bind(item: AmenityViewState.AmenityCheckbox) {
                binding.amenitiesCheckbox.setText(item.stringRes)
                binding.amenitiesCheckbox.setCompoundDrawablesWithIntrinsicBounds(item.iconDrawable, 0, 0, 0)
                binding.amenitiesCheckbox.setOnCheckedChangeListener { _, isChecked ->
                    item.onCheckBoxClicked.invoke(isChecked)
                }
                binding.amenitiesCheckbox.isChecked = item.isChecked
            }
        }
        ...
        class AmenityItem(private val binding: AmenityItemBinding) :
            ...
            fun bind(item: AmenityViewState.AmenityItem) {
                binding.amenityItemTv.setText(item.stringRes)
                binding.amenityItemTv.setCompoundDrawablesWithIntrinsicBounds(item.iconDrawable, 0, 0, 0)
            }
        }
